...
public class AugmentedFaceNode extends Node {
     ...
     */
    @SuppressWarnings({"initialization"})
    public AugmentedFaceNode(@Nullable AugmentedFace augmentedFace) {
        this();
...
//            Log.d(TAG, type + " " + Arrays.toString(position));
        ...
        if(faceRegionNode != null && faceRegionNode.getRenderableInstance() != null) {
            Objects.requireNonNull(faceRegionNode.getRenderableInstance().getFilamentAsset()).getInstance().getAnimator().updateBoneMatrices();
        }
    }
